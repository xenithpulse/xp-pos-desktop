...
foreach ($sub in @('app', 'node', 'mongodb\bin', 'caddy', 'service', 'scripts', 'config', 'redist', 'branding')) {
...
# -- branding ---------------------------------------------------------------
# Only the .ico goes into the payload, and it is needed there at RUN time, for
# two separate consumers: UninstallDisplayIcon in Add/Remove Programs, and the
# "XP POS" shortcut that provision.ps1 writes. setup.iss ALSO needs the same
# file at COMPILE time for SetupIconFile, but it reads that straight out of
# installer\branding - the two requirements are unrelated and satisfying only
# one of them leaves a broken icon somewhere.
#
# The wizard BMPs are deliberately NOT staged: they are compiled into the setup
# .exe and would be 2.3 MB of dead weight on every customer's disk. Neither is
# make-branding.ps1, which is a build-time tool.
$brandIco = Join-Path $InstallerDir 'branding\XP-POS.ico'
if (-not (Test-Path $brandIco)) {
    Write-Err "Branding icon missing: $brandIco"
    Write-Host "         Run installer\branding\make-branding.ps1 to regenerate it." -ForegroundColor DarkGray
    exit 1
}
Copy-Item $brandIco (Join-Path $OutDir 'branding\XP-POS.ico') -Force
Write-Ok "branding staged (XP-POS.ico)"
...
foreach ($rel in @(
    ...
    # the install - it just silently returns the product to a generic Windows
    # icon, which is the state this whole phase existed to get out of.
    'branding\XP-POS.ico'
)) {
...
# ── Branding ────────────────────────────────────────────────────────────────
# Assets are generated by installer\branding\make-branding.ps1 and checked in.
# These checks exist because every failure mode here is SILENT: a single-size
# icon still installs, a PNG named .bmp still compiles, and both are only
# noticed by a customer looking at a blurry icon or an empty wizard panel.

# A single-size .ico is the classic mistake - Windows scales the one image it
# finds and it looks blurry everywhere except at its native size.
$icoPath = Join-Path $OutDir 'branding\XP-POS.ico'
if (Test-Path $icoPath) {
    $icoBytes = [System.IO.File]::ReadAllBytes($icoPath)
    $icoSizes = @()
    if ($icoBytes.Length -ge 6 -and $icoBytes[2] -eq 1) {
        $count = [BitConverter]::ToUInt16($icoBytes, 4)
        for ($i = 0; $i -lt $count; $i++) {
            $entry = 6 + 16 * $i
            if ($entry + 16 -le $icoBytes.Length) {
                # A width byte of 0 means 256 - that is the format, not a bug.
                $w = $icoBytes[$entry]
                $icoSizes += $(if ($w -eq 0) { 256 } else { [int]$w })
            }
        }
    }
    Assert-Payload "the product icon is multi-resolution ($($icoSizes.Count) sizes)" `
        ($icoSizes.Count -ge 5) "found: $($icoSizes -join ', ')"
    Assert-Payload "the product icon covers 16px and 256px" `
        (($icoSizes -contains 16) -and ($icoSizes -contains 256)) `
        "16px is the taskbar and Explorer lists; 256px is extra-large view. Found: $($icoSizes -join ', ')"
}

# Compile-time assets. ISCC reads these straight out of installer\branding, so
# they are checked where they live rather than in the payload. Failing here
# gives a sentence that says what to do; failing in ISCC gives a line number.
$brandDir = Join-Path $InstallerDir 'branding'
foreach ($bmp in @(
    'WizardImage.bmp', 'WizardImage@2x.bmp', 'WizardImage@3x.bmp',
    'WizardSmallImage.bmp', 'WizardSmallImage@2x.bmp', 'WizardSmallImage@3x.bmp'
)) {
    $p = Join-Path $brandDir $bmp
    $exists = Test-Path $p
    Assert-Payload "wizard image present: $bmp" $exists `
        "run installer\branding\make-branding.ps1"
    if ($exists) {
        # Inno silently refuses a PNG here, leaving a blank wizard panel and no
        # error. Two bytes is all it takes to know.
        $sig = [System.IO.File]::ReadAllBytes($p)[0..1]
        Assert-Payload "$bmp is a real BMP" `
            ($sig[0] -eq 0x42 -and $sig[1] -eq 0x4D) `
            "Inno refuses PNGs renamed to .bmp, without reporting it"
    }
}

$licencePath = Join-Path $InstallerDir 'LICENSE.txt'
Assert-Payload "the licence page file exists" (Test-Path $licencePath) `
    "setup.iss references it via LicenseFile"

# WARNINGS, not assertions. A developer build must stay green with placeholders
# in place; what must not happen is shipping one to a paying customer without
# noticing. These fire on every build until the real values go in.
if (Test-Path $licencePath) {
    if ((Get-Content $licencePath -Raw) -match '\[REVIEW BEFORE SHIPPING\]') {
        Write-Warn "LICENSE.txt still has REVIEW BEFORE SHIPPING placeholders"
        Write-Host "         Jurisdiction and legal entity are unset. Not shippable." -ForegroundColor DarkGray
    }
}
$issText = Get-Content (Join-Path $InstallerDir 'setup.iss') -Raw
if ($issText -match 'xenithpulse\.com') {
    Write-Warn "setup.iss still has placeholder contact details"
    Write-Host "         SupportURL/SupportEmail/HomeURL/UpdatesURL at the top of the file." -ForegroundColor DarkGray
    Write-Host "         The same values live in config\brand.ts and provision.ps1." -ForegroundColor DarkGray
}
