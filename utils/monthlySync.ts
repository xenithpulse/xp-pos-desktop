...
/** Local (server-regional) human log timestamp. */
function tsPK() {
  return new Date().toLocaleString("en-GB") + " (local)";
}

/**
 * YYYY-MM-DD in the server's LOCAL (regional) timezone — the daily sheets are
 * keyed by local calendar day, so summaries must compare against the same.
 */
function ymdInTZ(date: Date | string): string {
...
/**
 * Normalize any incoming DailySheet (Document or POJO) into a clean IDailySheetForSync
 */
...
/**
 * Upsert the provided dailySheet doc into the currently OPEN MonthlySheet (isClosed=false).
 * Comparison is done by calendar date in Pakistan time (Asia/Karachi) to avoid TZ contradictions.
 */
...
export async function syncDailyToMonthlyByDailySheet(
  ...
  try {
    ...
    const dailyYMD = ymdInTZ(dailyDate);
    const monthStartYMD = ymdInTZ(new Date(monthly.startDate));
    const monthEndYMD = ymdInTZ(new Date(monthly.endDate));
    ...
    try {
      normalizedDailyId = new mongoose.Types.ObjectId(String(daily._id));
    } catch (err) {
